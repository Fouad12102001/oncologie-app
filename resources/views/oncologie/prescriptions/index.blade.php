...
<h1 style="color:#1a3e2b; font-weight:800; margin-bottom:16px;">📋 Toutes les Prescriptions</h1>
...
<form method="GET" action="{{ route('oncologie.prescriptions.index') }}"
    ...
    <input type="text" name="patient" placeholder="Nom du patient"
    ...
    <button type="submit"
            style="background:#2a9d8f; color:white; border:none; padding:10px 18px;
                   border-radius:10px; font-weight:600; cursor:pointer;">
        Filtrer
    </button>
    <a href="{{ route('oncologie.prescriptions.index') }}"
       style="background:#e5e7eb; color:#374151; padding:10px 16px; border-radius:10px;
...
<div style="display:flex; justify-content:space-between; align-items:center;
    ...
    <a href="{{ route('oncologie.prescriptions.create') }}"
       style="background:#2a9d8f; color:white; padding:10px 18px; border-radius:12px;
              text-decoration:none; font-weight:600; display:inline-flex; align-items:center; gap:6px;">
        <span class="material-icons" style="font-size:18px;">add</span> Ajouter Prescription
    </a>
    <div style="display:flex; gap:10px;">
        <a href="{{ route('oncologie.prescriptions.export') }}"
           style="background:#264653; color:white; padding:10px 18px; border-radius:12px;
                  text-decoration:none; font-weight:600; display:inline-flex; align-items:center; gap:6px;">
            <span class="material-icons" style="font-size:18px;">download</span> Exporter
        </a>
        <a href="{{ route('oncologie.prescriptions.stats') }}"
           style="background:#6a4c93; color:white; padding:10px 18px; border-radius:12px;
                  text-decoration:none; font-weight:600; display:inline-flex; align-items:center; gap:6px;">
            <span class="material-icons" style="font-size:18px;">bar_chart</span> Statistiques
        </a>
    </div>
</div>

{{-- TABLE --}}
<div style="background:white; border-radius:12px; border:1px solid #d1d5db;
            overflow-x:auto; box-shadow:0 4px 14px rgba(0,0,0,0.05);">
    <table style="width:100%; min-width:900px; border-collapse:collapse;">
        <thead style="background:#264653; color:#f4f8fb;">
            <tr>
                @foreach(['ID','Patient','Médicaments','SC (m²)','Dose (mg)','Médecin','Date','Statut','Actions'] as $h)
                    <th style="padding:12px; text-align:left; font-size:13px;">{{ $h }}</th>
                @endforeach
            </tr>
        </thead>
        <tbody>
            ...
            @empty
            <tr>
                <td colspan="9" style="padding:24px; text-align:center; color:#9ca3af;">
